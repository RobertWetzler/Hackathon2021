...
{
    //code taken from https://markheath.net/post/naudio-sine-portamento
    //uses a wavetable to smoothly transition between changes in frequency
    class SineWaveProvider : ISampleProvider
    {
        ...
        public SineWaveProvider(int sampleRate = 44100, double frequency = 1000f, double portamentoTime = 0.5)
        {
            ...
            for (int index = 0; index < sampleRate; ++index)
                ...
                //An attempt at square wave
                //waveTable[index] = (float)Math.Round((double)index / sampleRate);
